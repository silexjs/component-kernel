...
Kernel.prototype = {
	...
	searchBundle: function(value, service, file, error) {
		var self = this;
		return value.replace(/(\\)?@([A-Za-z0-9]+)/g, function(match, content1, content2, offset, s) {
			if(content1 === '\\') {
				return '@'+content2;
			}
			var bundle = self.getBundle(content2);
			if(bundle === null && error !== false) {
				service = service || null;
				file = file || null;
				throw new Error((service===null?'':'['+service+'] ')+'The "'+content2+'" bundle does not exist'+(file!==null?' in "'+file+'"':''));
			}
			return bundle.dir;
		});
	},
